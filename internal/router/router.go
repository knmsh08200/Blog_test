...
func NewHandler(ListProvider handlers.BlogMTVYHandlers) http.Handler {
	mux := mux.NewRouter()
	...
	mux.HandleFunc("/blog/list/{id}", ListProvider.BlogFindListHandler).Methods(http.MethodGet)
